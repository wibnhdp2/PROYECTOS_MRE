...
{
    public class TelevisorConsultaDA
    {
        ~TelevisorConsultaDA()
        {
            GC.Collect();
                                   ...
                                   ref int intTotalPaginas)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("PN_CLIENTE.USP_CL_TELEVISOR_CONSULTAR", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@telv_sOficinaConsularId", iOficinaconsularId));
