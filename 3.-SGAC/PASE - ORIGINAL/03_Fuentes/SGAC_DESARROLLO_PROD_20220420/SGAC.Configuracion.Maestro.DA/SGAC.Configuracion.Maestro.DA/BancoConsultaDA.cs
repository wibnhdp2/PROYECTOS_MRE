    ...
    public class BancoConsultaDA
    {
        ...
        public DataTable Consultar_Banco(short intBancoId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_BANCO_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_SBANCOID", intBancoId));
