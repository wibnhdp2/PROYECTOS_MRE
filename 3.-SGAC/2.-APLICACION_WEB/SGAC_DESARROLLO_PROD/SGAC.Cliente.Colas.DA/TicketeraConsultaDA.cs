...
{
    public class TicketeraConsultaDA
    {
        ~TicketeraConsultaDA()
        {
            GC.Collect();
                                   ...
                                   ref int intTotalPaginas)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("PN_CLIENTE.USP_CL_TICKETERA_CONSULTAR", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@tira_sOficinaConsularId", iOficinaconsularId));
