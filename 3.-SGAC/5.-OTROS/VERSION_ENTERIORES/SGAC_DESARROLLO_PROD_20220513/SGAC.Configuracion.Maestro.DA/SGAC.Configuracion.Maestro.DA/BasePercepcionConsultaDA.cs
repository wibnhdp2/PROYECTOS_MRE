...
{
    public class BasePercepcionConsultaDA
    {
        ~BasePercepcionConsultaDA()
        {
            GC.Collect();
        }
        string conexion()
        ...
        public DataTable Consultar_BasePercepcion(short intBasePercepcionId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_BASE_PERCEPCION_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_SBASEPERCEPCIONID", intBasePercepcionId));
