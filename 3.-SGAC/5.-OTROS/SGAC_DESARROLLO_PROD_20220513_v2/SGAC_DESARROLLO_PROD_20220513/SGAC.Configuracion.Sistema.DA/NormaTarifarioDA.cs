...
namespace SGAC.Configuracion.Sistema.DA
{
    public class NormaTarifarioDA
    {
        ...
        public DataTable ConsultarNorma(short intTipoNormaId, short intObjetoNormaId, string strDescripcionCortaNorma,
            string strFechaInicial, string strFechaFinal, short intEstadoId, short intGrupoNormaId, int intPageSize, int intPageNumber, 
            string strContar, ref int IntTotalCount, ref int IntTotalPages)
        {
            DataTable dtResultado = new DataTable();
            try
            {
                using (SqlConnection cn = new SqlConnection(this.conexion()))
                {
                    using (SqlCommand cmd = new SqlCommand("PS_SISTEMA.USP_SI_NORMA_CONSULTAR_MRE", cn))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@P_NORM_SGRUPONORMAID", intGrupoNormaId));
            ...
            try
            {
                using (SqlConnection cnx = new SqlConnection(this.conexion()))
                {
                    using (SqlCommand cmd = new SqlCommand("PS_SISTEMA.USP_SI_NORMA_ADICIONAR_MRE", cnx))
                    {
                        ...
                        cmd.Parameters.Add(new SqlParameter("@norm_sGrupoNormaId", objNorma.norm_sGrupoNormaId));
        ...
        //Motivo: Requerimiento Desarrollo SGAC_22_04_2022_EP
        //--------------------------------------------------------
        public DataTable ListaTitulosArticulosNorma(short intNormaId, short intTipoNormaId=0, short intObjetoNormaId=0, short intEstadoId=0, short intGrupoNormaId=0)
        {
            ...
            try
            {
                using (SqlConnection cn = new SqlConnection(this.conexion()))
                {
                    using (SqlCommand cmd = new SqlCommand("PS_SISTEMA.USP_SI_NORMA_LISTA_TITULOS_ARTICULOS_NORMAS_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(new SqlParameter("@P_NORM_SNORMAID", intNormaId));
                        cmd.Parameters.Add(new SqlParameter("@P_NORM_STIPONORMAID", intTipoNormaId));
                        cmd.Parameters.Add(new SqlParameter("@P_NORM_SOBJETONORMAID", intObjetoNormaId));
                        cmd.Parameters.Add(new SqlParameter("@P_NORM_SESTADOID", intEstadoId));
                        cmd.Parameters.Add(new SqlParameter("@P_NORM_SGRUPONORMAID", intGrupoNormaId));
