...
{
    public class RequisitoActuacionConsultaDA
    {
        ~RequisitoActuacionConsultaDA()
        {
            GC.Collect();
        }
        string conexion()
        ...
        public DataTable Consultar_RequisitoActuacion(short intRequisitoActuacionId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_REQUISITO_ACTUACION_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_SREQUISITOID", intRequisitoActuacionId));
