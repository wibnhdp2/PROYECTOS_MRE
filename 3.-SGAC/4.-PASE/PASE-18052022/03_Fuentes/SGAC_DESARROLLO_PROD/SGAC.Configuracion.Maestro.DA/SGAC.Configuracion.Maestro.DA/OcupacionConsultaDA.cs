...
{
    public class OcupacionConsultaDA
    {
        ~OcupacionConsultaDA()
        {
            GC.Collect();
        ...
        public DataTable Consultar_Ocupacion(int intOcupacionId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_OCUPACION_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_SOCUPACIONID", intOcupacionId));
