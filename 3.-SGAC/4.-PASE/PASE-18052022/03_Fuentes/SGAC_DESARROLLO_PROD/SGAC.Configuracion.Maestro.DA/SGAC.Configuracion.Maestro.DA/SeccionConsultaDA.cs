...
{
    public class SeccionConsultaDA
    {
        ~SeccionConsultaDA()
        {
            GC.Collect();     
        ...
        public DataTable Consultar_Seccion(short intSeccionId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_SECCION_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_SSECCIONID", intSeccionId));
