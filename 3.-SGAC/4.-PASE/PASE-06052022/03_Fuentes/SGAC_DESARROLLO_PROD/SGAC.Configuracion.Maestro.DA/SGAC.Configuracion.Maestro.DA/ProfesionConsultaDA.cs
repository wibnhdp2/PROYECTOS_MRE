...
{
    public class ProfesionConsultaDA
    {
        ~ProfesionConsultaDA()
        {
            GC.Collect();
        ...
        public DataTable Consultar_Profesion(short intProfesionId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_PROFESION_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_SPROFESIONID", intProfesionId));
