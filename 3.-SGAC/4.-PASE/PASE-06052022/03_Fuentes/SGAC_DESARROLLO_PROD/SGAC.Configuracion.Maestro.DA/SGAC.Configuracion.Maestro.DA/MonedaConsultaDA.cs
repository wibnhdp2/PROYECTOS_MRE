    ...
    public class MonedaConsultaDA
    {
        ...
        public DataTable Consultar_Moneda(int intMonedaId, string strEstado, string StrCurrentPage, int IntPageSize, string strContar, ref int IntTotalPages)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("SC_MAESTRO.USP_MA_MONEDA_CONSULTAR_MRE", cn))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;

                        cmd.Parameters.Add(new SqlParameter("@P_MONE_SMONEDAID", intMonedaId));
                        cmd.Parameters.Add(new SqlParameter("@P_MONE_CESTADO", strEstado));
