            ...
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    if (!Convert.ToBoolean(Session["Verifica"]))
                    {
                        ...
                        string ruta=VirtualPathUtility.ToAbsolute("~/Verificar.aspx");
                        ScriptManager.RegisterStartupScript(formulario.Page, typeof(string), "script", "window.parent.location='" + ruta + "';", true);
                        //Response.Redirect("~/Verificar.aspx");//se comenta esto por lo que redireccionaba dentro del iframe
                        return;
                    }
        ...
        protected void nextPage(object sender, EventArgs e)
        {
            try
            {
                ...
                if (exitoCaptcha)
                {
                    ...
                    {
                        Response.Redirect(@"..\01_Gen_Codigo\GeneraCodigo03.aspx", false);
                    }
                }
                else
                {
