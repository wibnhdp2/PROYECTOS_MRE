    ...
    public partial class GeneraCodigo03 : System.Web.UI.Page
    {
        ...
        protected void buscarSolicitud(object sender, EventArgs e)
        {
            try
            {
                ...
                List<beRegistroLinea> lista = obrRegistroLinea.consultarRegistroTrazabilidad(SessionGeneral);
                if (lista.Count > 0)
                {
                    SessionGeneral = lista[0];
                }
        ...
        protected void prevPage(object sender, EventArgs e)
        {
            try
            {
                Response.Redirect(@"..\01_Gen_Codigo\GeneraCodigo01.aspx",false);
            }
            catch(Exception ex)
            {
                //obrGeneral.grabarLog(ex);
