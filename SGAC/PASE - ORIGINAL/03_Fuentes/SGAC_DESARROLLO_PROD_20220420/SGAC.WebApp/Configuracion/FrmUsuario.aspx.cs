    ...
    { 
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session[Constantes.CONST_SESION_USUARIO_ROL_TIPO_ACCESO].ToString() == "LECTURA")
            {
                ...
                GridView[] arrGridView = { gdvUsuario };
        ...
        void ctrlToolBarMantenimiento_btnGrabarHandler()
        {
            ...
            else if (enmAccion == Enumerador.enmAccion.MODIFICAR)
            {
                ...
                if (HDCuentaRED.Value != "")
                {
                    if (HDCuentaRED.Value.ToUpper().Trim() != txtCuentaRedMant.Text.ToUpper().Trim())
                    {
                        DtUsro = UsroBL.Validar(HDCuentaRED.Value.ToUpper(), Convert.ToInt16(Session[Constantes.CONST_SESION_OFICINACONSULAR_ID]));
                        if (DtUsro.Rows.Count != 0)
                        {
                            strScript = Mensaje.MostrarMensaje(Enumerador.enmTipoMensaje.ERROR, strNombreEntidad, "El Alias ya se encuentra asignado a un rol.");
                            Comun.EjecutarScript(Page, strScript);
        ...
        private void CargarListadosDesplegables()
        {
            ...
            Util.CargarParametroDropDownList(ddlEntidadMant, comun_Part1.ObtenerParametrosPorGrupo(Session, Enumerador.enmGrupo.SEGURIDAD_ENTIDAD)); 
            ...
            Util.CargarParametroDropDownList(ddlAcceso, comun_Part1.ObtenerParametrosPorGrupo(Session, Enumerador.enmGrupo.SISTEMA_ACCESOS));
        }
        ...
        private void PintarSeleccionado(int intSeleccionado)
        {
            if (Session != null)
            {
                ...
                if (drSeleccionado != null)
                {
                    ...
                    HDCuentaRED.Value = drSeleccionado["usua_vAlias"].ToString();
                    ...
                    if (txtCuentaRedMant.Text.Trim() == StrSessionUsuario)
                    {
                        if (Convert.ToInt32(CtrlOficinaConsularConfig.SelectedValue) != (Convert.ToInt32(Session[Constantes.CONST_SESION_OFICINACONSULAR_ID])))
                        {
                            ...
                            {                                
                                ...
                                obj.Actualizar_Sesion_Bloqueada(intUsuarioId, false, getPAddress());
                                obj.Actualizar_Sesion_Activa(intUsuarioId, false, getPAddress());
                                //---------------------------------------------------
                            }
