...
BEGIN
	BEGIN TRY
		...
		BEGIN
			SELECT		
				RELI_SREG_LINEA_ID AS REGLINEA_ID,
				...
				(SELECT ESTA_VDESCRIPCIONCORTA FROM SC_MAESTRO.MA_ESTADO (NOLOCK) 
					WHERE ESTA_SESTADOID = RL.RELI_SESTADO_ID) AS RELI_ESTADO_DESC,						
				(SELECT ISNULL(PARA_VDESCRIPCION,'') FROM PS_SISTEMA.SI_PARAMETRO (NOLOCK) 
					WHERE PARA_SPARAMETROID = RL.RELI_SDP_GENERO_ID) AS RELI_GENERO_DESC,						
				(SELECT ISNULL(PARA_VDESCRIPCION,'') FROM PS_SISTEMA.SI_PARAMETRO (NOLOCK) 
					WHERE PARA_SPARAMETROID = RL.RELI_SDP_ESTADO_CIVIL_ID) AS RELI_ESTADOC_DESC,						
				(SELECT ISNULL(DOID_VDESCRIPCIONCORTA,'') FROM SC_MAESTRO.MA_DOCUMENTO_IDENTIDAD (NOLOCK) 
					WHERE DOID_STIPODOCUMENTOIDENTIDADID = RL.RELI_SDP_TIPO_DOC_IDENTIDAD) RELI_DOC_IDENT_DESC,
				(SELECT ISNULL(UBGE_VDEPARTAMENTO,'') FROM PS_SISTEMA.SI_UBICACIONGEOGRAFICA (NOLOCK)	
					WHERE UBGE_CUBI01 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,1,2)) AND UBGE_CUBI02 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,3,2)) AND UBGE_CUBI03 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,5,2))) AS RELI_DEPARTAMENTO,
				(SELECT ISNULL(UBGE_VPROVINCIA,'') FROM PS_SISTEMA.SI_UBICACIONGEOGRAFICA (NOLOCK) 
					WHERE UBGE_CUBI01 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,1,2)) AND UBGE_CUBI02 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,3,2)) AND UBGE_CUBI03 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,5,2))) AS RELI_PROVINCIA,
				(SELECT ISNULL(UBGE_VDISTRITO,'') FROM PS_SISTEMA.SI_UBICACIONGEOGRAFICA (NOLOCK) 
					WHERE UBGE_CUBI01 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,1,2)) AND UBGE_CUBI02 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,3,2)) AND UBGE_CUBI03 = (SELECT SUBSTRING(RELI_CDP_UBIGEO,5,2))) AS RELI_DISTRITO,						
				(SELECT PARA_VDESCRIPCION FROM PS_SISTEMA.SI_PARAMETRO (NOLOCK) 
					WHERE PARA_SPARAMETROID = RL.RELI_SDP_RELDEP_TITDEP) AS RELI_TITDEPDESC,
				ISNULL(CONVERT(VARCHAR,RELI_DDP_FECHA_NACIMIENTO,103),'-') AS RELI_FECNAC_STR,						
				(SELECT PAIS_VNOMBRE FROM PS_SISTEMA.SI_PAIS (NOLOCK)
					WHERE PAIS_SPAISID = RL.RELI_SDP_PAIS_NACIONALIDAD) AS RELI_PAIS,						
				(SELECT PARA_VDESCRIPCION FROM PS_SISTEMA.SI_PARAMETRO (NOLOCK) 
					WHERE PARA_SPARAMETROID = RL.RELI_STIPO_EMISION) AS RELI_TIPOEMISION,
		...
		BEGIN
			...
			SELECT @ESTADO_ENVIADO = ESTA_SESTADOID FROM SC_MAESTRO.MA_ESTADO WITH(NOLOCK) WHERE ESTA_VDESCRIPCIONCORTA = 'ENVIADO' AND ESTA_VGRUPO = 'REG LINEA - ESTADO' and ESTA_CESTADO = 'A'
			SELECT @ESTADO_APROBADO = ESTA_SESTADOID FROM SC_MAESTRO.MA_ESTADO WITH(NOLOCK) WHERE ESTA_VDESCRIPCIONCORTA = 'APROBADO' AND ESTA_VGRUPO = 'REG LINEA - ESTADO' and ESTA_CESTADO = 'A'
			SELECT @ESTADO_OBSERVADO = ESTA_SESTADOID FROM SC_MAESTRO.MA_ESTADO WITH(NOLOCK) WHERE ESTA_VDESCRIPCIONCORTA = 'OBSERVADO' AND ESTA_VGRUPO = 'REG LINEA - ESTADO' and ESTA_CESTADO = 'A'
			...
			SELECT		RELI_SREG_LINEA_ID AS REGLINEA_ID,
						...
						Isnull((SELECT Top 1 CAID_VCARNE_NUMERO FROM [SC_CARDIP].[CD_CARNE_IDENTIDAD] (NOLOCK)
									WHERE CAID_ICARNE_IDENTIDADID = RELI_SDP_RELDEP_TITULAR),'') AS NUMERO_CARNE_RELACION
			FROM		SC_REGLINEA.RL_REGISTRO_LINEA RL (NOLOCK)
			WHERE		RELI_VDP_NUMERO_REG_LINEA	=	CASE WHEN ISNULL(@P_RELI_VNUMERO_REG_LINEA,'') = '' THEN RELI_VDP_NUMERO_REG_LINEA 
														ELSE @P_RELI_VNUMERO_REG_LINEA END
