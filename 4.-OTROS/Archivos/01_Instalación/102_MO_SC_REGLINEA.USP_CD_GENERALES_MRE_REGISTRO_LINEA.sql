...
GO
	...
	BEGIN
		...
		BEGIN TRY
			...
			FROM	PS_SISTEMA.SI_PARAMETRO PARA_GENERO WITH(NOLOCK)
			...
			FROM	PS_SISTEMA.SI_PARAMETRO PARA_ESTCIV WITH(NOLOCK)
			...
			FROM	PS_SISTEMA.SI_PAIS PAIS WITH(NOLOCK)
			...
			FROM	PS_SISTEMA.SI_PARAMETRO PARA_TITDEP WITH(NOLOCK)
			...
			FROM	PS_SISTEMA.SI_PARAMETRO TIPO_EMI WITH(NOLOCK)
			...
			FROM	PS_SISTEMA.SI_PARAMETRO TIPO_INST WITH(NOLOCK)
			...
			FROM	SC_MAESTRO.MA_DOCUMENTO_IDENTIDAD DOCIDENT WITH(NOLOCK)
			...
			FROM	SC_MAESTRO.MA_CALIDAD_MIGRATORIA CALMIG WITH(NOLOCK)
			...
			FROM	SC_MAESTRO.MA_CALIDAD_MIGRATORIA CALMIG WITH(NOLOCK)
			INNER JOIN PS_SISTEMA.SI_PARAMETRO TITDEP WITH(NOLOCK) ON CALMIG.CAMI_SFLAG_TITULAR_DEPENDIENTE=TITDEP.PARA_SPARAMETROID
			WHERE	CALMIG.CAMI_CESTADO='A' AND
			...
			FROM		PS_SISTEMA.SI_OFICINACONSULAR_EXTRANJERA OFCOEX WITH(NOLOCK)
