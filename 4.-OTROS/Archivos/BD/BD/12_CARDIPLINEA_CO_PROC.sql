...
SET NOCOUNT ON;
	...
	BEGIN TRY
      ...
      SELECT @LOER_VNOMFORMULARIO,@LOER_VERROR,@LOER_VDETALLEERROR,@LOER_VMETODO,@LOER_SUSUARIOCREACION,@LOER_VIPCREACION,
			 @FECHA
	END TRY
	BEGIN CATCH
...
BEGIN
	BEGIN TRY
		...
		IF NOT EXISTS( SELECT DERE_SCODIGO FROM SC_REGLINEA.RL_DETALLE_REGISTRO_LINEA WHERE DERE_SREG_LINEA_ID=@P_RELI_IREGISTRO_LINEA_ID AND DERE_SESTADO_ID=@P_RELI_SESTADO_ID)BEGIN
			DECLARE @NUEVO_DERE_SCODIGO SMALLINT
			SELECT @NUEVO_DERE_SCODIGO= (ISNULL(MAX(DERE_SCODIGO),0)+1) FROM SC_REGLINEA.RL_DETALLE_REGISTRO_LINEA WHERE DERE_SREG_LINEA_ID=@P_RELI_IREGISTRO_LINEA_ID
